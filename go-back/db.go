...
func InitializeDb() {
	...
	database.Exec("CREATE TABLE Availability(candidateId TEXT, votingId TEXT, FOREIGN KEY (candidateId) REFERENCES Candidate(id) ON DELETE CASCADE ON UPDATE CASCADE FOREIGN KEY(votingId) REFERENCES Voting(id) ON DELETE CASCADE ON UPDATE CASCADE PRIMARY KEY (candidateId, votingId));")
	database.Exec("CREATE TABLE Vote(id TEXT PRIMARY KEY, votingId TEXT, candidateId TEXT, token TEXT, FOREIGN KEY (candidateId) REFERENCES Candidate(id) ON DELETE CASCADE ON UPDATE CASCADE FOREIGN KEY (votingId) REFERENCES Voting(id) ON DELETE CASCADE ON UPDATE CASCADE FOREIGN KEY (token) REFERENCES Token(value) ON DELETE CASCADE ON UPDATE CASCADE);")
